...
os.chdir(os.getcwd())#<===The location of your csv files
...
#    regex =  'bold;\">\s*Item 1\.(.+?)bold;\">\s*Item 1A\.'
    ...
    for i in range(len(toc)-1):
        ...
        if match:
            ...
            raw = soup.text.encode('utf8')
            out = raw.strip()
            fname = txtSubPath+file_name.replace(".htm",'')
            f = open(fname+'_'+toc[i]+'.txt','w')
            f.write(out)
            f.close()
